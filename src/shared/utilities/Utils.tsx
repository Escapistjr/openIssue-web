...
export function elementChildren<T extends React.ReactElement<{}>>(
	...
	return React.Children.toArray(children).filter((child) => React.isValidElement(child) && predicate(child as T)) as T[];
}

function hotReloadComponentCheck(AComponent: React.ComponentType<any>, AnotherComponent: React.ComponentType<any>) {
	const componentName = AComponent.name;
	const anotherComponentName = (AnotherComponent as React.StatelessComponent<any>).displayName;
...
const isComponent =
	process.env.NODE_ENV === 'development'
		...
		: (AComponent: React.ComponentType<any>, AnotherComponent: React.ComponentType<any>) => AComponent === AnotherComponent;
